guidebook-window-title = Путівник
guidebook-placeholder-text = Виберіть запис.
guidebook-placeholder-text-2 = Якщо ви новачок, перейдіть до розділу "Новий? Почніть тут!"
guidebook-filter-placeholder-text = Фільтр елементів

guidebook-parser-error = Помилка аналізатора
guidebook-error-message = Повідомлення про помилку

guidebook-monkey-unspin = Unspin Monkey
guidebook-monkey-disco = Диско Мавпа

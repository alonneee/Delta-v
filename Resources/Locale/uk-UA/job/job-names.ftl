job-name-atmostech = Атмосферний Інженер
job-name-bartender = Бармен
job-name-borg = Кіборг
job-name-botanist = Ботанік
# DeltaV - Changed Brigmedic to Corpsman
#job-name-brigmedic = Brigmedic
job-name-cadet = Кадет
job-name-captain = Капітан
job-name-cargotech = Вантажник
job-name-cburn = Офіцер карантину CentComm
job-name-ce = Головний Інженер
job-name-centcommoff = Офіційний представник CentComm
job-name-chef = Шеф-кухар
job-name-chaplain = Капелан
job-name-chemist = Хімік
job-name-clown = Клоун
job-name-cluwne = Cluwne
job-name-cmo = Головний Лікар
job-name-deathsquad = Ескадрон смерті
job-name-detective = Детектив
job-name-doctor = Лікар
job-name-engineer = Інженер
job-name-ertchaplain = Капелан ERT
job-name-ertengineer = ERT Інженер
job-name-ertjanitor = ERT Прибиральник
job-name-ertleader = ERT Лідер
job-name-ertmedic = ERT Медик
job-name-ertsecurity = ERT Охоронець
job-name-hop = Голова Персоналу
job-name-hos = Голова Безпеки
job-name-intern = Медичний Інтерн
job-name-janitor = Прибиральник
# DeltaV - Changed Lawyer to Attorney
#job-name-lawyer = Lawyer
# DeltaV - Changed Librarian to Archivist
#job-name-librarian = Librarian
job-name-mime = Мім
job-name-musician = Музикант
job-name-paramedic = Парамедик
# DeltaV - Changes Passenger to Assistant. job-name-passenger is used in a dozen other places as the default job so we dont want to change that overly much.
job-name-passenger = Пасажир
job-name-psychologist = Психолог
# DeltaV - Hopefully makes LO maintaining easier
# job-name-qm = Quartermaster
# DeltaV - Changed RD to Mystagogue
# job-name-rd = Research Director
job-name-reporter = Журналіст
job-name-research-assistant = Науковий Асистент
job-name-salvagespec = Шахтар
job-name-scientist = Вчений
job-name-security = Офіцер
job-name-serviceworker = Сервісний працівник
job-name-station-ai = Станція АІ
job-name-technical-assistant = Технічний Асистент
job-name-visitor = Відвідувач
job-name-warden = Варден

# unused jobs
# these are required for the agent ID job icon tooltips
# I am keeping them for roleplaying opportunities
job-name-geneticist = генетик
#job-name-boxer = Boxer # DeltaV - Keeping boxer/Zookeper job
job-name-no-id = Без посвідчення особи
job-name-prisoner = Ув'язнений
job-name-roboticist = Робототехнік
job-name-unknown = Невідомий
job-name-virologist = { 0 }{ CAPITALIZE(SUBJECT($target)) } { CONJUGATE-HAVE($target)} символізується через { POSS-ADJ($target)} тіло.{ 1 }
#job-name-zookeeper = Zookeeper # DeltaV - Keeping Boxer/Zookeper job

# antagonist jobs
job-name-ninja = Ніндзя
job-name-syndicate = Синдикат
job-name-syndicate-commander = Командир синдикату
job-name-syndicate-corpsman = Корпус синдикату
job-name-syndicate-operative = Оперативний синдикат
job-name-pirate = Пірат
job-name-wizard = майстер
job-name-zombie = зомбі

# Job titles
job-title-cluwne = Cluwne
job-title-universal = Універсальний
job-title-visitor = Відвідувач

# Role timers - Make these alphabetical or I cut you
JobAtmosphericTechnician = Атмосферний Інженер
JobBartender = Бармен
JobBorg = Кіборг
JobBotanist = Ботанік
JobCaptain = Капітан
JobCargoTechnician = Вантажник
JobCentralCommandOfficial = Представник ЦК
JobChaplain = Капелан
JobChef = Шеф-кухар
JobChemist = Хімік
JobChiefEngineer = Головний Інженер
JobChiefMedicalOfficer = Головний Лікар
JobClown = Клоун
JobDetective = Детектив
JobBrigmedic = Медик Служби Безпеки
JobERTChaplain = Капелан ERT
JobERTEngineer = ERT інженер
JobERTJanitor = ERT прибиральник
JobERTLeader = ERT Лідер
JobERTMedical = ERT медик
JobERTSecurity = ERT офіцерСБ
JobHeadOfPersonnel = Голова Персоналу
JobHeadOfSecurity = Голова Безпеки
JobJanitor = Прибиральник
JobLawyer = Адвокат
JobLibrarian = Бібліотекар
JobMedicalDoctor = Лікар
JobMedicalIntern = Інтерн
JobMime = Мім
JobMusician = Музикант
JobParamedic = Парамедик
JobPassenger = Пасажир
JobPsychologist = Психолог
JobQuartermaster = Голова Карго
JobReporter = Журналіст
JobResearchAssistant = Науковий Асистент
JobResearchDirector = Директор Наукового Відділу
JobSalvageSpecialist = Шахтар
JobScientist = Вчений
JobSecurityCadet = Кадет
JobSecurityOfficer = Офіцер
JobServiceWorker = Сервісний працівник
JobStationAi = Станція АІ
JobStationEngineer = Інженер
JobTechnicalAssistant = Технічний Асистент
JobVisitor = Відвідувач
JobWarden = Варден

cheat-death-component-examined = [color=red]Щось підказує вам, що {SUBJECT($target)} не боїться смерті. [/color]
cheat-death-component-remaining-revives = { $->
[0] You have no revives left.
[one] You have one revive left.
*[other] You have {$amountamount->
[0] You have no revives left.
[one] You have one revive left.
*[other] You have {$amount } revives left.
}

cheat-death-component-remaining-revives-unlimited = Ви маєте необмежену кількість оживлень.

action-cheat-death-fail-no-lives = Цього разу ти не вислизаєш.
action-cheat-death-fail-not-dead = Смерть живою не обдуриш!
action-cheat-death-holy-damage = Божественне втручання перешкоджає вашому відродженню.

action-cheated-death-dead = {$name}рвучко прокидається. Кістки виправлені, рани зашиті, органи влаштовані настільки ідеально.
action-cheated-death-alive = {$name}сильно тремтить, усілякі рани зникають у спалаху полум’я.